...
interface JoinFormProps {
	gameCode: string;
	teams: Team[];
	onJoined: (data: { playerId: string; playerToken: string }) => void;
}
...
export function JoinForm({ gameCode, teams, onJoined }: JoinFormProps) {
	...
	async function handleJoin() {
		...
		try {
			...
			onJoined({ playerId: data.playerId, playerToken: data.playerToken });
		} catch {
	...
	return (
		<div className="w-full max-w-sm mx-auto space-y-6">
			<div className="text-center space-y-1">
				...
				<p className="text-sm text-muted">Playing as <span className="text-foreground font-medium">{displayName}</span></p>
			</div>
			...
			{error && (
				<p className="text-sm text-answer-red text-center">{error}</p>
			)}
