...
export async function POST(request: Request) {
	try {
		...
		if (!body.hostPin || body.hostPin.length < 4) {
			return Response.json({ error: "Host PIN must be at least 4 characters" }, { status: 400 });
		}
		...
		if (!validateQuestions(body.questions)) {
			return Response.json(
				{ error: "Invalid questions format. Each question needs text, 4 options, a correctOptionId, and timeLimit (5-120s)." },
				{ status: 400 },
			);
		}
