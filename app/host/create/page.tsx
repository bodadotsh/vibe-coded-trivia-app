...
export default function CreateGame() {
		...
		try {
			...
			for (let i = 0; i < parsedQuestions.length; i++) {
				...
				if (!q.timeLimit || q.timeLimit < 5 || q.timeLimit > 120) {
					parsedQuestions[i] = { ...q, timeLimit: 20 }; // Default to 20s
				}
			}
	...
	async function handleSubmit(e: React.FormEvent) {
		...
		try {
			...
			sessionStorage.setItem(`host-token-${data.gameCode}`, data.hostToken);
			...
			<div className="w-full max-w-2xl space-y-8">
				...
				<form onSubmit={handleSubmit} className="space-y-6">
					...
					<div className="grid grid-cols-1 gap-4 sm:grid-cols-2">
						<div className="space-y-2">
							<label htmlFor="hostPin" className="block text-sm font-medium text-muted">
								Host PIN
							</label>
							<input
								id="hostPin"
								...
								className="w-full rounded-xl border border-card-border bg-card px-4 py-3 text-foreground placeholder:text-muted focus:border-accent focus:outline-none"
							/>
							<p className="text-xs text-muted">Only you need this to manage the game</p>
						</div>
						<div className="space-y-2">
							...
							{questions.length > 0 ? (
								...
								<div className="space-y-1">
									...
									<p className="text-xs text-muted">
										Each question: text, 4 options, correctOptionId, timeLimit
									</p>
								</div>
							)}
						</button>
